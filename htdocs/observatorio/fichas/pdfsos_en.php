...
$query0  = "select  a.resumen_en, a.fecha_ultima_edicion, dp.titulo_en tit_proyecto, a.id_catalogo_etapa id_etapa_sost, c.Titulo_En tit_etapa_sost ";
$query0 .= ", dp.id_sector, s.DESCRIPCION_EN tit_sector, dp.id_subsector, su.DESCRIPCION_EN tit_subsector ";
...
$resumen_sostenibilidad_proy_en = $_row['resumen_en'];
if (mb_strlen($resumen_sostenibilidad_proy_en) > 407)
    $resumen_sostenibilidad_proy_en = substr($resumen_sostenibilidad_proy_en, 0, 407) . '<a target="_blank" href="'.(isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]/?post_type=proyecto_inversion&#038;p=".$id_post."&language=en&so=1#collapseSostenibilidad".'">&nbsp;Show more...</a>';
...
$fuentes = "";
$tot_fuentes = 0;
$result = $conn_ob->query("select documento_en from tbl_documentos_x_ficha where id_fichas_sostenibilidad = $id_sostenibilidad order by id");
$num_fuentes = $result->num_rows;
while ($row = mysqli_fetch_array($result)) {
    $fuente = $row['documento_en'];
...
if (mb_strlen($fuentes) > 545){
    ...
    $fuentes = $fuentes . '<a target="_blank" href="'.(isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]/?post_type=proyecto_inversion&#038;p=".$id_post."&language=en&so=1#collapseSostenibilidad".'">&nbsp;Show more...</a>';
}

#DATOS DE PROTAL PROYECTOS MEXICO   
$url_qr_bid_en = _query("SELECT meta_value FROM wp_postmeta WHERE post_id=57797 and meta_key='url_qr_bid_en'");
$resumen_bid_en = _query("SELECT meta_value FROM wp_postmeta WHERE post_id=57797 and meta_key='resumen_bid_en'");
$metodologia_proyecto_en = _query("SELECT meta_value FROM wp_postmeta WHERE post_id=57797 and meta_key='metodologia_proyecto_en'");
$consulta_proyecto_PM_en = _query("SELECT meta_value FROM wp_postmeta WHERE post_id=57797 and meta_key='consulta_proyecto_PM_en'");
$marco_metodologico_bid_en = _query("SELECT meta_value FROM wp_postmeta WHERE post_id=57797 and meta_key='marco_metodologico_bid_en'");
$titulo_resumen_sostenibilidad_ingles = _query("SELECT meta_value FROM wp_postmeta WHERE post_id=57797 and meta_key='titulo_resumen_sostenibilidad_ingles'");


# QR
$qrcode = "../../wp-content/cache/tmp/resources/qrcode/";
//$FileName = 'EN_' . $id_post . '_PRO.png';
//$qr = new qrcode(); // No se usa, se usa QRcode::png() directamente
//$qr_size = 150; // No se usa
//$qr_level = 'L'; // No se usa
//$qr_margin = '0'; // No se usa

# QR BID ESPANIOL
$FileName_bid = 'EN_' . $id_post . '_BID.png';
...
# QRCODE
//$qr_url_proyecto_en = "../../wp-content/cache/tmp/resources/qrcode/EN_" . $id_post . "_PRO.png";
$qr_bid_en = $ruta_content."/cache/tmp/resources/qrcode/".$FileName_bid;
...
$output_file_name_2 = $output_file_name."EN_" . $id_sostenibilidad . "_SOS_1.pdf";
...
function Dimensiones($conexion, $id, $etapa){
    ...
    while ($row_0 = $rsl_0->fetch_assoc()) {
        ...
        switch ($id_dimension){
            case 1: $img = "Financiera_en.png";
            break;
            case 2: $img = "Ambiental_en.png";
            break;
            case 3: $img = "Social_en.png";
            break;
            case 4: $img = "Institucional_en.png";
            break;
        }
        ...
        $rsl_1 = $conn_ob->query("SELECT practica_en FROM tbl_buenas_practicas_x_ficha WHERE id_fichas_sostenibilidad=".$id." AND id_catalogo_dimensiones=".$id_dimension);
                ...
                <tr>
                    <td style='position: sticky;width:100px;'>					
                        ...
                        <div style='padding:5px;border-right:1px solid #008588;'>
                            <b style='color:#008588;text-transform:uppercase;'>EXAMPLE OF GOOD PRACTICES</b>					
                            <div>" . $buenas_practicas['practica_en'] . "</div>
                        </div>
                    </td>
                    <td style='width:555px;" . $border . "'>
                        ".Atributos($conn_ob, $id, $id_dimension)."
                    </td>
                </tr>
            </table>
        </div>";
...
function Atributos($conexion, $id, $id_dimension){
    //$conn_ob = $conexion->conexionMysql();
    # Colores
    $C1 = "#DDDDDD";
    $C2 = "#008588";
    ...
    <table>
        <tr style='border:6px solid #fff'>
            <td style='border-left:2px; text-align:center; width:455px;'><strong style='color:#008588;'>Sustainability criteria<strong></td> ";
            // Query para obtener el alias de los TIERS ND,T1,T2,T3
            $queryt = "SELECT alias_en, Valor FROM tbl_catalogo_tiers WHERE Valor > -1 order by Valor";
    ...
    $html .= "
        ...
        $query = "select atr.Nombre_En as attr , ti.Valor as valor   
