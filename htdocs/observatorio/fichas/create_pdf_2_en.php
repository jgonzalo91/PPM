...
$Gradial = isset($_POST['radial']) ? $_POST['radial'] : '';
...
$output_file_name = "/wp-content/cache/tmp/pdf_sostenbilidad/EN_" . $id_ficha_sostenibilidad . "_SOS_2.pdf";
...
// Unir el archivo de BID y GIZ
$pdf_bid = $_SERVER['DOCUMENT_ROOT'].'/wp-content/cache/tmp/pdf_sostenbilidad/EN_'.$id_ficha_sostenibilidad.'_SOS_1.pdf';
if(file_exists($pdf_bid)){
    $unir = new PDFMerger();
    $unir->addPDF($pdf_bid, 'all');
    $unir->addPDF($output_file_path, 'all');
    $file_proyecto  = $_SERVER['DOCUMENT_ROOT'].'/wp-content/cache/tmp/pdf_detalle_proyectos/EN_'.$id_post.'.pdf';
    if(file_exists($file_proyecto)){
        $unir->addPDF($file_proyecto, 'all');
    }
    $final = $_SERVER['DOCUMENT_ROOT'].'/wp-content/cache/tmp/pdf_sostenbilidad/EN_'.$id_ficha_sostenibilidad.'_SOS.pdf';
