...
$output .= '<div class="toggle_content invers-color" itemprop="text" style="display:none; background-color: #3b3e40;">';
	...
	/*function shortcode_conteo_procura_auto() {
    $conn = new conexion();
    $conOb = $conn->conexionMysql();
    ...
    $sector_filtro = isset($filtros_array['sector']) ? intval($filtros_array['sector']) : 0;
    $subsector_filtro = isset($filtros_array['subsector']) ? $filtros_array['subsector'] : 0;

    // Construir condiciones para SQL según filtros disponibles
    $condiciones = [];

    if ($sector_filtro) {
        $condiciones[] = "id_sector = $sector_filtro";
    }

    if ($subsector_filtro) {
        if (is_array($subsector_filtro)) {
            $subsector_ids = array_map('intval', $subsector_filtro);
            $subsector_list = implode(',', $subsector_ids);
            $condiciones[] = "id_subsector IN ($subsector_list)";
        } else {
            $subsector_id = intval($subsector_filtro);
            $condiciones[] = "id_subsector = $subsector_id";
        }
    }

    if (empty($condiciones)) {
        return '<p>Error: No hay filtros válidos para sector o subsector.</p>';
    }
    ...
    $sql_conteo = "
        SELECT
          (SELECT COUNT(DISTINCT id_proyecto)
           FROM tbl_procura
           WHERE $where_sql) AS total_proyectos,
          (SELECT COUNT(DISTINCT empresa_es)
           FROM tbl_procura
           WHERE $where_sql) AS total_empresas,
          (SELECT COUNT(DISTINCT id_propuesta)
           FROM tbl_procura
           WHERE $where_sql AND tipo_participante = 'Consorcio') AS total_consorcios
    ";
    ...
    $output .= '<p><strong>Total Proyectos:</strong> ' . number_format($total_proyectos) . '</p>';
    $output .= '<p><strong>Total Empresas:</strong> ' . number_format($total_empresas) . '</p>';
    $output .= '<p><strong>Total Consorcios:</strong> ' . number_format($total_consorcios) . '</p>';
    $output .= '</div>';

    return $output;
}
add_shortcode('conteo_procura_auto', 'shortcode_conteo_procura_auto');*/
